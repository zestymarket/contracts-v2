...
describe('ZestyMarket_ERC20_V1_2', function() {
  let signers;
  let zestyMarket;
  let timeNow;
  ...
  beforeEach(async () => {
    ...
    let ZestyMarket = await ethers.getContractFactory('ZestyMarket_ERC20_V1_2');
  ...
  it('[batched] It should allow a buyer to cancel a bid if the seller has not approved', async function() {
    ...
    for (let i=1; i<=3; i++) {
      ...
      expect(data.privateBuyer).to.equal(ethers.constants.AddressZero);
      expect(data.saleType).to.equal(1);
    }
  ...
  it('[batched] It should allow an operator to reject the ad if autoApprove is disabled', async function() {
    ...
    for (let i=1; i<=3; i++) {
      ...
      expect(data.buyerCampaignApproved).to.equal(0);
      expect(data.privateBuyer).to.equal(ethers.constants.AddressZero);
      expect(data.saleType).to.equal(0);
    }
