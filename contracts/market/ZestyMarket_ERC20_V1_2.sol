...
contract ZestyMarket_ERC20_V1_1 is ZestyVault, ReentrancyGuard {
    ...
    struct BuyerCampaign {
        uint256 buyerCampaignId; // this is the token id
        address buyer;
        uint256 inProgressCount;
    }
    mapping (uint256 => BuyerCampaign) private _buyerCampaigns;

    event BuyerCampaignCreate(uint256 indexed buyerCampaignId, address buyer);
    event BuyerCampaignUpdate(uint256 indexed buyerCampaignId, uint256 inProgressCount);
    event BuyerCampaignWithdraw(uint256 indexed buyerCampaignId);
        ...
        )
    {
        ...
        inProgressCount = _buyerCampaigns[_buyerCampaignId].inProgressCount;
    }
    ...
    function buyerCampaignCreate(uint256 _tokenId) external nonReentrant {
        _depositZestyNFT(_tokenId);

        _buyerCampaigns[_tokenId] = BuyerCampaign(
            _tokenId,
            msg.sender,
            0
        );
    ...
    function buyerCampaignWithdraw(uint256 _tokenId) external nonReentrant {
        require(
            _buyerCampaigns[_tokenId].inProgressCount == 0, 
            "ZestyMarket_ERC20_V1::buyerCampaignWithdraw: Auction or Contact is in progress"
        );
        _withdrawZestyNFT(_tokenId);
        delete _buyerCampaigns[_tokenId];
        emit BuyerCampaignWithdraw(_tokenId);
    }
    ...
    function sellerNFTWithdraw(uint256 _tokenId) external onlyDepositor(_tokenId) nonReentrant {
        ...
        require(
            s.inProgressCount == 0, 
            "ZestyMarket_ERC20_V1::sellerNFTWithdraw: Auction or Contact is in progress"
        );
        _withdrawZestyNFT(_tokenId);
    ...
    function sellerAuctionBidBatch(uint256[] memory _sellerAuctionId, uint256 _buyerCampaignId) external nonReentrant {
        for (uint i=0; i < _sellerAuctionId.length; i++) {
            ...
            _buyerCampaigns[_buyerCampaignId].inProgressCount = _buyerCampaigns[_buyerCampaignId].inProgressCount.add(1);

            emit BuyerCampaignUpdate(_buyerCampaignId, _buyerCampaigns[_buyerCampaignId].inProgressCount);
    ...
    function sellerAuctionBidCancelBatch(uint256[] memory _sellerAuctionId) external nonReentrant {
        for (uint i=0; i < _sellerAuctionId.length; i++) {
            ...
            b.inProgressCount = b.inProgressCount.sub(1);
            emit BuyerCampaignUpdate(b.buyerCampaignId, b.inProgressCount);
    ...
    function sellerAuctionRejectBatch(uint256[] memory _sellerAuctionId) external nonReentrant {
        for (uint i=0; i < _sellerAuctionId.length; i++) {
            ...
            _buyerCampaigns[s.buyerCampaign].inProgressCount = _buyerCampaigns[s.buyerCampaign].inProgressCount.sub(1);
            emit BuyerCampaignUpdate(s.buyerCampaign, _buyerCampaigns[s.buyerCampaign].inProgressCount);
    ...
    function contractWithdrawBatch(uint256[] memory _contractId) external nonReentrant {
        for(uint i=0; i < _contractId.length; i++) {
            ...
            _buyerCampaigns[c.buyerCampaignId].inProgressCount = _buyerCampaigns[c.buyerCampaignId].inProgressCount.sub(1);
            emit BuyerCampaignUpdate(s.buyerCampaign, _buyerCampaigns[s.buyerCampaign].inProgressCount);
